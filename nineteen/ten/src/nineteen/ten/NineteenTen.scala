...
object NineteenTen extends App with Support {
  ...
  def run(data: String) = {
    ...
    def detections(c: Coord) = {
      ...
      val normed = candidates.groupBy(cand => {
        val dx = cand.x - c.x
        val dy = cand.y - c.y
        val g = Math.abs( gcd(dx, dy) )
        Coord(dx / g, dy / g)
      }).view.mapValues(_.toSeq.sortBy(_.manhattan(Coord(0, 0)))).toMap
      c -> normed
    }
...
//    println(p1._2)
    ...
    val ordered = p1._2.map {

      case (k, v) => Math.atan2(k.x.toDouble, k.y.toDouble) -> v
    }.toSeq.sortBy(_._1).map(_._2.sortBy(_.manhattan(p1._1))).reverse
