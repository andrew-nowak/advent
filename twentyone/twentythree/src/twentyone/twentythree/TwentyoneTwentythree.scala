package twentyone.twentythree

import lib.Support
import twentyone.twentythree.State.roomEntrances
...
final case class State(
  ...
  private def repeat(a: Amphipod): List[Amphipod] = List.fill(roomDepth)(a)
  def situated = ((roomA == repeat(Amber)) :: (roomB == repeat(Bronze)) :: (roomC == repeat(
    Copper
  )) :: (roomD == repeat(Desert)) :: Nil).count(identity)
...
object TwentyoneTwentythree extends App with Support {
  @tailrec
